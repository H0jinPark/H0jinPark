<h1 align="center">👋 Hi, I'm Hojin Park</h1>
<p align="center">
  <b>Data Science & AI Engineering · Multimodal ML · Time-Series · NLP</b><br>
  산업공학 · 데이터사이언스 기반의 문제 해결형 AI 엔지니어
</p>

---

## 🧑‍💻 About Me
SKKU 산업공학 + 데이터사이언스 융합전공 기반으로  
**데이터 분석 → ML/DL 모델링 → MLOps 실험 환경 구축 → 연구/논문 발표**까지  
엔드-투-엔드 경험을 갖춘 AI/ML 엔지니어입니다.

📌 주요 경험  
- 시계열 예측(수요·환경 데이터)  
- NLP 기반 욕설 탐지 & 순화 모델 개발  
- K-Fashion 감성 텍스트 기반 이미지 생성 모델(ControlNet)  
- 도시 vs 비도시 **주민 정보 요구 차이 연구**, ASIS&T Short Paper 발표  
- 최적화 기반 JSSP(공정 스케줄링) + Node2Vec 임베딩  
- AutoML, 대규모 ML 실험 관리, HuggingFace 기반 모델 운영  

데이터로 문제를 정의하고, ML을 통해 해결하며,  
“실제로 동작하는 결과물”을 만들어내는 데 관심이 있습니다.

---

## 🛠 Tech Stack

### 🔥 AI / ML
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white)
![HuggingFace](https://img.shields.io/badge/HuggingFace-F7931E?style=for-the-badge&logo=huggingface&logoColor=white)
![scikit-learn](https://img.shields.io/badge/scikitlearn-F7931E?style=for-the-badge&logo=scikitlearn&logoColor=white)

### 📊 Data / MLOps
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-00648B?style=for-the-badge&logo=mysql&logoColor=white)
![AutoML](https://img.shields.io/badge/AutoML-0A66C2?style=for-the-badge&logo=automl&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)

### 📝 Collaboration
![Notion](https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white)
![Slack](https://img.shields.io/badge/Slack-4A154B?style=for-the-badge&logo=slack&logoColor=white)

---
